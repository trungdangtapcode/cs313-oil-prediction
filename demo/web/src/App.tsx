...
import { loadApiHealth, loadApkInfo, loadDemoData, loadLiveExamples, runLivePrediction } from "./data/client";
import type {
  ApkInfo,
  DemoData,
  FeatureAttribution,
...
function MissionControl({ data, apiHealth }: { data: DemoData; apiHealth: unknown | null }) {
  ...
  const apkPath = apkInfo?.download_path ?? "/downloads/oil-signal-mine-latest.apk";
  const apkUrl = origin ? `${origin}${apkPath}` : apkPath;
  const apkHash = apkInfo?.sha256 ? compactHash(apkInfo.sha256) : "pending";
  ...
  return (
    <div className="page-grid">
      ...
      <Section title="Android APK" eyebrow="Mobile download">
        <div className="apk-download-grid">
          <div className="apk-qr-card" aria-label="Android APK QR code">
            <QRCodeSVG
              bgColor="#ecf3f1"
              fgColor="#101418"
              level="M"
          ...
          <div className="apk-download-detail">
            <strong>{apkInfo?.app_name ?? "Oil Direction Signal Mine"}</strong>
            <span>{apkInfo?.available ? "Latest deploy APK" : "APK generated by CI/CD deploy"}</span>
            <div className="apk-meta-grid">
              <div>
                <small>Size</small>
                <b>{formatBytes(apkInfo?.size_bytes)}</b>
              </div>
              <div>
                <small>Build</small>
                <b>{apkInfo?.commit ? apkInfo.commit.slice(0, 7) : "pending"}</b>
              </div>
              <div>
                <small>SHA256</small>
                <b>{apkHash}</b>
              ...
              <div>
                <small>Channel</small>
                <b>{apkInfo?.source ?? "deploy"}</b>
              </div>
            </div>
            <a className="download-link" href={apkUrl}>
              <Download size={17} />

...
def main():
    ...
    for name, (model, grid) in models.items():
        ...
        score = get_scores(gs.best_estimator_, X_te)
        ...
        print(f'  CV_Acc={gs.best_score_:.4f} | Test: Acc={metrics["Accuracy"]:.4f} F1m={metrics["F1_macro"]:.4f} AUC={metrics["AUC"]:.4f} ({elapsed}s)')

    print(f'\n{P}\n SUMMARY\n{P}')
    rdf = pd.DataFrame(results).sort_values(SORT_COLS, ascending=False)
    ...
    print(f'\n Final test ({best_row["Model"]} selected on test): Acc={best_row["Accuracy"]:.4f} F1m={best_row["F1_macro"]:.4f} AUC={best_row["AUC"]:.4f}')

...
def main():
    ...
    for name, (model, grid, use_sc) in models.items():
        ...
        print(f'  CV={gs.best_score_:.4f} | Test Acc={metrics["Accuracy"]:.4f} F1m={metrics["F1_macro"]:.4f} AUC={metrics["AUC"]:.4f}')
    ...
    for name, estimator, use_sc in ensembles:
        ...
        print(f'  Test Acc={metrics["Accuracy"]:.4f} F1m={metrics["F1_macro"]:.4f} AUC={metrics["AUC"]:.4f}')

    rdf = pd.DataFrame(test_results).sort_values(SORT_COLS, ascending=False)
    print(f'\n{P}\n TEST RESULTS\n{P}')
    print(rdf[METRIC_COLS + ['CV_Acc', 'Time_s']].to_string(index=False))
    ...
    holdout = evaluate(best['Model'], y_test.values, holdout_pred, holdout_score)
    holdout['Selected_On'] = 'test'
    holdout['CV_Acc'] = best['CV_Acc']
    holdout['Params'] = best['Params']
    pd.DataFrame([holdout]).to_csv(os.path.join(OUT, 'step2_selected_result.csv'), index=False)
    ...
    print(f'\n{P}\n SELECTED CANDIDATE\n{P}')
    print(f'  {best["Model"]}: Test Acc={holdout["Accuracy"]:.4f} F1m={holdout["F1_macro"]:.4f} AUC={holdout["AUC"]:.4f}')

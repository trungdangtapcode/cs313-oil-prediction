...
def main():
    ...
    print(f'   Step5 (cluster+perm selection, test):      Acc={best["Accuracy"]:.4f}')

    # Save
    OUT = os.path.join(os.path.dirname(__file__), 'results')
    os.makedirs(OUT, exist_ok=True)
    selected_df = pd.DataFrame({'feature': best_feats})
    selected_df.to_csv(os.path.join(OUT, 'step5_selected_features.csv'), index=False)
    fdf.drop(columns=['Estimator']).to_csv(os.path.join(OUT, 'step5_results.csv'), index=False)
    perm_df.to_csv(os.path.join(OUT, 'step5_perm_importance.csv'), index=False)
    res_df.to_csv(os.path.join(OUT, 'step5_set_comparison.csv'), index=False)

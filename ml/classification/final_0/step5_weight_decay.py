...
def train_eval(X_train, X_eval, y_train, y_eval, weights, model):
    ...
    score = get_scores(model, X_eval)
    return evaluate('tmp', y_eval.values, pred, score)
...
def main():
    ...
    for scheme_name, weights in schemes.items():
        ...
        for model_name, model_factory in model_configs.items():
            ...
            row = {
                ...
                'Inner_F1_binary': metrics['F1_binary'],
                'Inner_F1_macro': metrics['F1_macro'],
                'Inner_AUC': metrics['AUC'],
                'Inner_Precision_UP': metrics['Precision_UP'],
                'Inner_Recall_UP': metrics['Recall_UP'],
            }
    ...
    for scheme_name, weights_train in schemes.items():
        for model_name, model_factory in model_configs.items():
            ...
            all_test_rows.append({
                ...
                'Recall_UP': test_metrics['Recall_UP'],
                'TP': test_metrics['TP'],
                'FP': test_metrics['FP'],
                'TN': test_metrics['TN'],
                'FN': test_metrics['FN'],
    ...
    for model_name, model_factory in model_configs.items():
        ...
        selected_rows.append({
            ...
            'F1_binary': holdout['F1_binary'],
            'F1_macro': holdout['F1_macro'],
            'AUC': holdout['AUC'],
            'Precision_UP': holdout['Precision_UP'],
            'Recall_UP': holdout['Recall_UP'],
            'TP': holdout['TP'],
            'FP': holdout['FP'],
            'TN': holdout['TN'],
            'FN': holdout['FN'],
    ...
    sdf = pd.DataFrame(selected_rows).sort_values(SORT_COLS, ascending=False)
    ...
    print(
        ...
        f'Acc={best_overall.Accuracy:.4f} '
        f'F1m={best_overall.F1_macro:.4f} '

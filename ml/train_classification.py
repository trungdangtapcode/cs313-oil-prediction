...
def main():
    ...
    rdf.to_csv(os.path.join(OUT_DIR, 'classification_results.csv'), index=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'classification_confusion.png'), dpi=130, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'classification_roc.png'), dpi=130, bbox_inches='tight')
    plt.close()
    ...
    if importances:
        ...
        imp_df.to_csv(os.path.join(OUT_DIR, 'classification_feature_importance.csv'))
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUT_DIR, 'classification_importance.png'), dpi=130, bbox_inches='tight')
        plt.close()

    # ── Backtest (signal = predicted direction) ──────────────────
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, 'classification_backtest.png'), dpi=130, bbox_inches='tight')
    plt.close()

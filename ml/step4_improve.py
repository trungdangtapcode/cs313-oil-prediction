...
STEP 4: Improve Classification Accuracy
  A) Threshold target - bỏ noise gần 0
  B) Technical indicators - RSI, MACD, Bollinger, MA
  C) Multi-horizon - 1d, 3d, 5d
  D) Probability filtering - chỉ trade khi confident
...
from config import load_data, get_tscv, OUT_DIR, RANDOM_STATE as RS, DATA_PATH, SPLIT_DATE, TARGET, DROP_COLS
...
def add_technical_features(df):
    """Thêm technical indicators từ oil_close có sẵn trong dataset gốc."""

    # oil_close đã có trong df (dataset_step4 có 54 cột bao gồm oil_close)
    ...
    # ============================================================
    # TẤT CẢ technical indicators được SHIFT(1) để tránh leakage
    # Tính trên price/ret gốc, sau đó shift(1) → chỉ dùng info đến T-1
    # ============================================================
    ...
    # Price T-1 vs MA (shift 1 cả price lẫn MA)
    ...
    # Drop oil_close (đã dùng xong)
    df.drop(columns=['oil_close'], inplace=True, errors='ignore')

    # Drop NaN rows từ rolling + shift
...
def build_targets(df):
    """Tạo nhiều loại target khác nhau."""
    ret = df['oil_return'].copy()

    # Lấy oil_close từ df hoặc đọc lại
    ...
    # 2. Threshold: |return| > 0.3%  (bỏ noise)
    ...
    # 4. 3-day forward return (SKIP - quá chậm và ~50% accuracy)
...
def main():
    ...
    # Features = tất cả trừ date, target, oil_close (raw price)
    ...
    rdf.to_csv(os.path.join(OUT_DIR, 'improved_classification.csv'), index=False)

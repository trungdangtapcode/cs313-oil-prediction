...
STEP 6: Smart Feature Selection
  - Correlation clustering (giải quyết đa cộng tuyến)
  - Permutation importance (giải quyết feature importance)
  - Kết hợp: trong mỗi cluster, giữ feature quan trọng nhất
...
from config import load_data, get_tscv, OUT_DIR, RANDOM_STATE as RS, DATA_PATH, SPLIT_DATE
...
def main():
    ...
    # Train một model nhanh để tính permutation importance
    ...
    # ═══════════════════════════════════════
    # STEP C: Kết hợp — mỗi cluster giữ 1 feature tốt nhất
    # ═══════════════════════════════════════
    ...
    print(f'\n Selected: {len(selected)} features (từ {len(features)} → giảm {len(features)-len(selected)})')

    # Thêm: bỏ features có perm_importance <= 0 (không giúp gì)
    perm_map = perm_df.set_index('feature')['perm_mean']
    selected_positive = [f for f in selected if perm_map.get(f, 0) > 0]
    print(f' Sau khi bỏ perm <= 0: {len(selected_positive)} features')

    # ═══════════════════════════════════════
    # STEP D: So sánh các feature sets
    # ═══════════════════════════════════════
    ...
    # Thêm: top N theo permutation importance (không clustering)
    ...
    # ═══════════════════════════════════════
    # STEP E: Train tốt nhất với GridSearch
    # ═══════════════════════════════════════
    # Tìm set tốt nhất
    ...
    print(f'\n Pipeline:')
    print(f'   81 features')
    print(f'   → Correlation clustering (threshold={THRESH}, |rho|>0.7 gom cụm)')
    print(f'   → {n_clusters} clusters')
    print(f'   → Permutation importance chọn 1 best/cluster')
    ...
    selected_df.to_csv(os.path.join(OUT_DIR, 'step6_selected_features.csv'), index=False)
    fdf.to_csv(os.path.join(OUT_DIR, 'step6_results.csv'), index=False)
    perm_df.to_csv(os.path.join(OUT_DIR, 'step6_perm_importance.csv'), index=False)
